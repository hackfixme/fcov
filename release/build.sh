...
# Build fcov binaries for Linux, macOS and Windows, packaging them in their
# platform-native formats.
# Based on https://github.com/hackfixme/disco/blob/c3066dd3726f16cce2b617641755270f75387106/release/build.sh
...
build() {
  ...
  if [ -n "$_version" ]; then
    _build_args+=(-ldflags "-X go.hackfix.me/fcov/app/context.vcsVersion=${_version}")
  fi
...
package() {
  ...
  deb|rpm)
    ...
    cd "$_path"
    set -x
    nfpm package --config ./nfpm.yaml --packager "${_fmt}" \
      --target "${_dest_dir}/${_name}.${_fmt}"
    set +x
    cd -
    ;;
  tgz)
  ...
  for _pf in $_platforms; do
    ...
    case "$_pf" in
      ...
      macos*)
        ...
        ;;
      windows*)
        if [ "$_goarch" = "amd64" ] || [ "$_pf" = "windows" ]; then
          build GOOS=windows,GOARCH=amd64 _build_path
          package "$_build_path" "zip"
        fi
        if [ "$_goarch" = "arm64" ] || [ "$_pf" = "windows" ]; then
          build GOOS=windows,GOARCH=arm64 _build_path
          package "$_build_path" "zip"
        fi
        ;;
      *)
